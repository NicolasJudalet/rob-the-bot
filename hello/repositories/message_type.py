...
from hello.models import MessageType

def get_hydrated_message_body(message_type_id):
    MESSAGE_TEMPLATES_DIR = "{}/hello/api/payload_templates".format(
        BASE_DIR
    )

    message_type = get(message_type_id)
    message_template_file = os.path.join(MESSAGE_TEMPLATES_DIR, message_type.template_file)

    with open(message_template_file) as f:
        message_body = json.dumps(json.load(f)).replace(
